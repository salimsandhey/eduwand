          ...
          <>
            ...
            {!isTopicMode || classSubTab === "overview" ? (
              <>
                ...
                <View style={[styles.card, styles.overallCard, { backgroundColor: colors.surface }, cardShadow]}>
...
function ReportNote({ title, value, colors }: { title: string; value: string | null; colors: ReturnType<typeof useTheme>["colors"] }) {
  const { cardShadow } = useTheme();
  return <View style={[styles.reportNote, { backgroundColor: colors.surface }, cardShadow]}><Text style={[styles.reportNoteTitle, { color: colors.textPrimary }]}>{title}</Text><Text style={[styles.reportNoteBody, { color: colors.textSecondary }]}>{value ?? "Not recorded yet."}</Text></View>;
}
  ...
  return (
    <View>
      {topics.map((topic) => (
        <Pressable
          key={topic.topicId}
          style={({ pressed }) => [styles.topicRow, { backgroundColor: colors.surface }, cardShadow, pressed && { opacity: pressedOpacity }]}
...
const styles = StyleSheet.create({
  ...
  studentRow: { minHeight: 70, borderRadius: 16, paddingHorizontal: 13, flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 9 }, studentCopy: { flex: 1 }, studentName: { fontSize: 13, fontWeight: "800" }, studentMeta: { marginTop: 2, fontSize: 11, fontWeight: "500" }, studentScore: { fontSize: 16, fontWeight: "800" },
  topicRow: { minHeight: 66, borderRadius: 16, paddingHorizontal: 15, flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 9 }, topicRowCopy: { flex: 1 },
