...
export class PlanExpiredError extends AiLimitError {
  constructor(readonly status: "expired" | "none") {
    super(
      "plan_expired",
      undefined,
      status === "none" ? "You don't have an active plan, so AI features are paused." : "Your plan has ended, so AI features are paused."
    );
