...
export interface PresentationContent {
  ...
  // Only set when template is "school_format" - set in code from the
  // school's branding row, never generated by the model.
  logoUrl?: string | null;
  ...
  slides: {
    title: string;
    bullets: string[];
    // Only set when template is "more_visual" - best-effort, a slide with no
    // good Unsplash match simply has none (falls back to the color scheme).
    imageUrl?: string;
  }[];
}
    ...
    switch (outputType) {
      ...
      case "presentation":
        content = {
          ...
          colorScheme: presentationColorScheme ?? "indigo",
          slides: [
...
// colorScheme never reaches this: it's a pure display preference the teacher
// picks, set directly on the stored content, not something the model decides.
const PRESENTATION_TEMPLATE_INSTRUCTIONS: Record<PresentationTemplate, string> = {
  detailed:
    "Make this a text-heavy, detailed reference deck: 4-6 bullet points per slide, each bullet a full " +
    "explanatory sentence (not a fragment), suitable for a student reading it independently without a teacher present.",
  instructional:
    "Structure this as step-by-step instructions for a process: each slide is exactly ONE sequential step. " +
    'Every slide title MUST start with "Step N: " (N = that slide\'s position, starting at 1). Bullets under ' +
    "each step are the concrete actions/details for that step only, in the order they should be performed.",
  // Branding (logo/colors) is a rendering concern applied after generation,
  // not a content-structure one - same instructions as "detailed" is correct.
  school_format:
    "Make this a text-heavy, detailed reference deck: 4-6 bullet points per slide, each bullet a full " +
    "explanatory sentence (not a fragment), suitable for a student reading it independently without a teacher present.",
  more_visual:
    "Keep this punchy and visual: 2-3 short bullet points per slide (not full sentences - short phrases), " +
    "since each slide will be built around a large photo and doesn't have room for dense text.",
};
...
class GeminiAiProvider implements AiProvider {
  ...
  async generateContent({
    ...
    // colorScheme is a pure display preference, never seen by the model -
    // set here alongside template so both are always present/correct
    ...
    if (outputType === "presentation" && parsed && typeof parsed === "object") {
      ...
      (parsed as Record<string, unknown>).colorScheme = presentationColorScheme ?? "indigo";
    }
